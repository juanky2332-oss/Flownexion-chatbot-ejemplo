...
const RealisticRobot: React.FC<RealisticRobotProps> = ({ className, isPointing = false, size = 64, onlyHead = false }) => {
  // FIX: Generamos un ID único para este robot específico.
  // Esto evita que los colores de un robot se mezclen con los de otro.
  ...
  return (
    <div className={`relative flex items-center justify-center ${className}`} style={{ width: size, height: size }}>
      <svg 
        ...
        // FIX: 'overflow-visible' permite que las sombras y efectos salgan del marco sin cortarse
        ...
        <g transform={onlyHead ? "translate(0, 40)" : "translate(0, 0)"}>
          {!onlyHead && (
            <>
              ...
              {/* FIX: Usamos la función grad() para referenciar los IDs únicos */}
              <rect x="65" y="115" width="70" height="55" rx="25" fill={`url(#${grad("robotWhite")})`} />
          ...
          <g>
            <rect x="45" y="30" width="110" height="95" rx="45" fill={`url(#${grad("robotWhite")})`} />
